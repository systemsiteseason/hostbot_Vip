    ...
    m_Ready(true)
{
  ...
  string   IRC_PrivateKey     = CFG->GetString("irc_privatekey", "MobaVietnam");
  string   IRC_LimitUser      = CFG->GetString("irc_limituser", "100");
  string   IRC_GameMaster     = CFG->GetString("irc_gamemaster", "MobaVietnamMaster");
  ...
  if (IRC_Server.empty() || IRC_NickName.empty() || IRC_Port == 0 || IRC_Port >= 65535)
    ...
    m_IRC = new CIRC(this, IRC_Server, IRC_NickName, IRC_UserName, IRC_Password, IRC_PrivateKey, IRC_LimitUser, IRC_GameMaster, IRC_Channels, IRC_RootAdmins, IRC_Port, IRC_CommandTrigger[0]);
