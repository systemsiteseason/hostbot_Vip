...
CIRC::CIRC(CAura* nAura, string nServer, const string& nNickname, const string& nUsername, string nPassword, string nPrivateKey, string nLimitUser, string nGameMaster, vector<string> nChannels, vector<string> nRootAdmins, uint16_t nPort, int8_t nCommandTrigger)
  : m_Aura(nAura),
    ...
    m_PrivateKey(std::move(nPrivateKey)),
    m_LimitUser(nLimitUser),
    m_GameMaster(nGameMaster),
  ...
  for (auto& Packets_Packet : Packets)
  {
    ...
    if (Tokens.size() >= 2 && Tokens[1] == "376")
    {
      ...
      for (auto& channel : m_Channels){
        SendIRC("JOIN " + channel + " " + m_PrivateKey);
        SendIRC("MODE " + channel + " +nklo " + m_PrivateKey + " " + m_LimitUser + " " + m_GameMaster);
      }
